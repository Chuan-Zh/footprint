#!/usr/bin/env perl
#
#Transfer hongwei's socre to operon level by direct mapping, e.g. map a gene to operon it
#is in, two genes inside an operon not consider to be in a regulon
#
use strict;
use warnings;
use Data::Dumper;
...
while(<IN>) {
  ...
  my ($opr1, $opr2);
  if (defined $id2first_gene_in_operon{$g1}) {
    $opr1 = $id2first_gene_in_operon{$g1};
  } else {
    $opr1 = $g1;
  }

  if (defined $id2first_gene_in_operon{$g2}) {
