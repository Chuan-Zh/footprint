...
def ppt_gi2strand(file='NC_000913.ptt'):
    ''' read ppt file, return gi to strand'''
    fin = open(file)
    fin.readlines(3)
    rv = {}
    for line in fin:
...
def opr_leading_gene(gi2strand, file='NC_000913.opr'):
    ''' read opr file, return ang gi to the opr gi set it is in'''
    fin = open(file)
    rv = set()
    for l in fin:
        ...
        if gi2strand.has_key(gis[0]) and gi2strand[gis[0]] == '+':
            rv.add(gis[0])
        elif gi2strand.has_key(gis[0]) and gi2strand[gis[0]] == '-':
            rv.add(gis[-1])
...
if __name__ == "__main__":
    ...
    GENE_COUNT = 4146
    OPR_COUNT = 2405 # the ones that has a leagel leading gene
    P_CUTOFF = 1e-5

    biclu = read_bicluster(exp_f)
    bic_p = read_bicluster_positive(exp_f)
    b2gi = ppt_b2gi()
    gi2strand = ppt_gi2strand()
    leading_genes = opr_leading_gene(gi2strand)
    ...
    biclusters = []
    for clu in biclu:
        gis = [b2gi[b] for b in clu if b2gi.has_key(b) and (b2gi[b] in leading_genes) ]
